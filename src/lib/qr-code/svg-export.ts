...
export function buildSvgExportString(input: SvgExportInput): string {
  ...
  // No frame: the QR's background is the preset's container colour. Frame
  // present: the frame's own backgroundColor governs the outer area.
  if (!config.frame && input.outerBackground) {
